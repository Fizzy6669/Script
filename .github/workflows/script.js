...
// выбор уборок 
x.cleanings = {
    1: { name: 'Быстрая',       summa: 1500   },
    2: { name: 'Генеральная',   summa: 6000   },
    3: { name: 'После ремонта', summa: 8000   },
}

// выбор комнат
x.rooms = { 
    1: { name: '1 комната', summa: 1000, text: 'Уборка квартиры с 1 жилой комнатой' },
    2: { name: '2 комнаты', summa: 3000, text: 'Уборка квартиры с 2 жилыми комнатами' },
    3: { name: '3 комнаты', summa: 7000, text: 'Уборка квартиры с 3 жилыми комнатами' },
...
let sendform = () => {
    ...
    out(j => {
	  ...
	  let check__head = $('.check b').html();

	  let check__list =  $('#check-list').html().replace(/<[^>]+>/g,' ');
	  let check__list_clear_1 = check__list.replace(/отмена/gi,'\n');
	  let check__list_clear_2 = check__list_clear_1.replace(/ +/g, ' ').trim();

	  let total = $('#c-total').html().replace(/<[^>]+>/g,' ');
	  let date = $('#t-a-9').val();
	  let time = $('#t-a-8').val();

		$.ajax({
			type: 'POST',
			url: 'https://uborka-rybinsk.com/mail.php',
			data: {
				check__head: check__head,
				check__list: check__list_clear_2,
				total: total,
				name: x.name,
				phone: x.phone,
				date: date,
		...
		document.getElementById('t-a-4').value = null;
		x.name = '';
		document.getElementById('t-a-5').value = '+7(___)___-__-__';
		x.phone = '';
		document.getElementById('t-a-6').value = null;
		x.code = '';
		document.getElementById('t-a-7').value = null;
		x.comment = '';
